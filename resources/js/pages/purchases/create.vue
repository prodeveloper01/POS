...
export default {
    ...
    methods: {
        ...
        // get products
        async getProducts() {
            const { data } = await this.form.get(window.location.origin + "/api/all-products");
        ...
        // get taxes
        async getTaxes() {
            const { data } = await this.form.get(window.location.origin + "/api/all-vat-rates");
            this.taxes = data.data;
            // assign default vat rate
            if (this.taxes && this.taxes.length > 0) {
                let defaultVatRateSlug = this.appInfo.defaultVatRateSlug;
                this.form.orderTax = this.taxes.find((tax) => tax.slug === defaultVatRateSlug);
            }
            this.updateTax();
        },
        // get accounts
        ...
        storeProduct(product) {
            var index = this.form.selectedProducts.findIndex((x) => x.id == product.id);
            let qunatity = 1;
            if (index === -1) {
                let purchasePrice = product.avgPurchasePrice > 0 ? product.avgPurchasePrice : 1;
                let productTax = product.taxType == "Exclusive"
                ...
                this.form.selectedProducts.unshift({
                    ...
                    unitCost: product.taxType == "Exclusive"
                        ? purchasePrice + totalTax
                        : purchasePrice,
                    totalPrice: product.taxType == "Exclusive"
        ...
        calculateSum() {
            ...
            if (this.form.subTotal > 0) {
                this.form.netTotal = Number((this.form.subTotal +
                    Number(this.form.totalTax) +
                    Number(this.form.transportCost) -
                    Number(this.form.discount)).toFixed(2));
            }
            return;
        },
        // save purchase
    ...
    components: { SupplierCreateModal }
};
</script>
